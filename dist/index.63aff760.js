...
function showWarningUsername(input) {
    if (input.target.value.includes("@")) userNameWarning.textContent = `Gebruikersnaam mag geen @ bevatten`;
    else userNameWarning.textContent = "";
}
function showWarningPassword(input) {
    if (input.target.value === "" || input.target.value.length > 6) passwordWarning.textContent = "";
    else passwordWarning.textContent = `Wachtwoord is te kort, gebruik minimaal 6 tekens`;
...
},{}]},["gvwj5","adjPd"], "adjPd", "parcelRequire401f")
